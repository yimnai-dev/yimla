...
export const get = async <TData = unknown>({ url, fetcher = fetch, baseURL, options }: GetRequestOptions): Promise<TData> => {
    const reqURL = `${baseURL}/${url}`
    const response = await fetcher(reqURL, {
        ...options
    })
...
export const deleteRequest = async <TData = unknown>({ url, fetcher = fetch, baseURL, options }: GetRequestOptions): Promise<TData> => {
    const reqURL = `${baseURL}/${url}`
    const response = await fetcher(reqURL, {
        ...options,
        method: 'DELETE',
