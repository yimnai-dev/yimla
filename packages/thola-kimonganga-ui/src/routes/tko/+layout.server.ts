...
import { getDeviceLocationInfo, getOrgMedicationList, getPharmacistList, getPharmacyList, getSubscriptionList } from '$lib/utils'
...
export const load = async ({ locals, cookies, fetch, setHeaders, platform }) => {
    ...
    return {
        ...
        tko: {
            organisationPharmacistListResponse: getPharmacistList({ cookies, fetcher: fetch, baseURL: locals.baseURL, url: `pharmacist/org/all/${orgDetailsResponse.organisation.organisationId}` }),
            ...
            pharmacyListStream: getPharmacyList({ cookies, fetcher: fetch, orgId: orgDetailsResponse.organisation.organisationId, baseURL: locals.baseURL }),
            subscriptionListStream: getSubscriptionList({ cookies, fetcher: fetch, customerId: orgDetailsResponse.organisation.customerId, baseURL: locals.baseURL }),
            medicationListStream: getOrgMedicationList({ cookies, fetcher: fetch, organisationId: orgDetailsResponse.organisation.organisationId, baseURL: locals.baseURL }),
