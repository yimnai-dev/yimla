...
func SendForgotPasswordEmailHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		jsonRes := utils.EncodedApiError("Wrong Request Format", http.StatusNotFound)
		w.WriteHeader(http.StatusNotFound)
		w.Write(jsonRes)
		return
	}
	if user.Email == "" {
		jsonRes := utils.EncodedApiError("Email is required", http.StatusNotFound)
		w.WriteHeader(http.StatusNotFound)
		w.Write(jsonRes)
		return
	}
	...
	if err != nil {
		jsonRes := utils.EncodedApiError("Wrong Email Format", http.StatusNotFound)
		w.WriteHeader(http.StatusNotFound)
		w.Write(jsonRes)
		return
	}
	...
	if err != nil && err == sql.ErrNoRows {
		jsonRes := utils.EncodedApiError("Account does not exist. Make sure you entered the correct email", http.StatusNotFound)
		w.WriteHeader(http.StatusNotFound)
		w.Write(jsonRes)
		return
	}
	...
	if err != nil {
		...
		w.Write(utils.EncodedApiError(err.Error(), http.StatusInternalServerError))
		return
	}
...
func VerifyEmailHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		w.WriteHeader(http.StatusNotFound)
		w.Write(utils.EncodedApiError("Wrong Request Format", http.StatusNotFound))
		return
	}
	if user.Email == "" {
...
func ResetAccountPasswordHandler(w http.ResponseWriter, r *http.Request) {
