...
func UpdateMedication(w http.ResponseWriter, r *http.Request) {
	...
	updateStockQuery := `UPDATE drug_stocks SET price = COALESCE($1, price, price), quantity = COALESCE($2, quantity, quantity) WHERE drug_id = $3`
	...
	updateDrugQuery := `UPDATE drugs SET expiry_date = COALESCE($1, expiry_date, expiry_date), instructions = COALESCE($2, instructions, instructions), storage_conditions = COALESCE($3, storage_conditions, storage_conditions), description = COALESCE($4, description, description) WHERE drug_id = $5`
...
func GetMedicationDetails(w http.ResponseWriter, r *http.Request) {
...
func GetPharmacyMedications(w http.ResponseWriter, r *http.Request) {
	var medications []MedicationDetails = []MedicationDetails{}
	pharmacyId := chi.URLParam(r, "pharmacyId")
	pharmacyMedicationListQuery := `SELECT drugs.drug_id, drugs.name, drugs.description, drugs.manufacturer, drugs.expiry_date, drugs.created_on, drugs.updated_on, drugs.category, drugs.strength, drugs.dosage_form, drug_stocks.stock_id, drug_stocks.quantity, drug_stocks.price FROM drugs LEFT JOIN drug_stocks ON drugs.drug_id = drug_stocks.drug_id LEFT JOIN pharmacies ON drugs.pharmacy_id = pharmacies.pharmacy_id WHERE drugs.pharmacy_id = $1`
